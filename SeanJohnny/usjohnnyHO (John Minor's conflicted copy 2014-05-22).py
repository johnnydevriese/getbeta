...
L = ( -2.0 * np.eye(s) + np.eye(s,k=-1)+np.eye(s,k=1) ) / ((G/2)/s)**2
...
Omega = [E[k] - E[0] for k in range(20)] 
Omega = [Omega[k] / 4 for k in range(20)]  
#Omega = range(20)
...
B_max = (3.0**0.25)*(((e*h)/(m**0.5))**3) * 1./ (1.)**(7/2)
